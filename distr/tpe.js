...
const At=I`<svg class="icon" height="24" viewBox="0 0 24 24" width="24"><path d="M0 0h24v24H0z" fill="none"/><path d="M13 7h-2v4H7v2h4v4h2v-4h4v-2h-4V7zm-1-5C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm0 18c-4.41 0-8-3.59-8-8s3.59-8 8-8 8 3.59 8 8-3.59 8-8 8z"/></svg>`;class St extends(tt(ht(J))){static get styles(){return[super.styles,a`
    ...
    `}constructor(){super(),this._boundKeyEventListener=this._eventListener.bind(this)}static get properties(){return{submitOnEnter:{type:Boolean,attribute:"submit-on-enter"}}}_eventListener(t){this.form&&13===t.keyCode&&(1===this.form.elements.length||this.submitOnEnter)&&this.form.submit()}afterSettingProperty(t,e){super.afterSettingProperty(t,e),"value"===t&&this.internals&&this.internals.setFormValue(this.value)}firstUpdated(){super.firstUpdated(),this.addEventListener("keydown",this._boundKeyEventListener);const t=this.shadowRoot.querySelector("#datalist-slot"),e=t&&t.assignedElements()[0],i=e&&e.children;if(i&&i.length){const t=document.createElement("datalist");t.setAttribute("id","_datalist"),this.setAttribute("list","_datalist");for(const e of i){const i=document.createElement("option");i.setAttribute("value",e.getAttribute("value")),t.appendChild(i)}this.shadowRoot.appendChild(t)}}}t.register("nn-input-text",Bt);t.register("nn-input-color",class extends Bt{constructor(){super(),this.type="color"}});t.register("nn-input-date",class extends Bt{constructor(){super(),this.type="date"}});t.register("nn-input-datetime-local",class extends Bt{constructor(){super(),this.type="datetime-local"}});t.register("nn-input-email",class extends Bt{constructor(){super(),this.type="email"}});class Ut extends(ut(tt(ct(ht(J))))){static get styles(){return[super.styles,a`
      ...
      ${this.fileName}
    `}fileNameChanged(t){const e=this.shadowRoot.querySelector("#native"),i=e.value;e.files.length>1?(this.fileName=this.manyFilesText+` (${e.files.length})`,this.title=Array.from(e.files).map((t=>t.name)).join("\n")):1===e.files.length?(this.fileName=i.slice(i.lastIndexOf("\\")+1),this.title=this.fileName):(this.fileName="",this.title="")}}t.register("nn-input-file",Ut);t.register("nn-input-month",class extends Bt{constructor(){super(),this.type="month"}});t.register("nn-input-number",class extends Bt{constructor(){super(),this.type="number"}});t.register("nn-input-password",class extends Bt{constructor(){super(),this.type="password"}});class Ht extends(ut(pt(Z(tt(ct(ht(J))))))){render(){return I`
      ${this.ifLabelBefore}
      ${this.ifValidationMessageBefore}
      <input as-radio value-source="checked" @change="${this._excludeOthers}" type="radio" id="native" real-time-event="input">
      ${this.ifValidationMessageAfter}
      ${this.ifLabelAfter}
    `}firstUpdated(){super.firstUpdated(),this.setAttribute("type","radio")}_excludeOthers(t){const e=[...this.form.children].filter((t=>t!==this&&!!t.form&&t.getAttribute("name")&&t.getAttribute("name")===this.getAttribute("name")&&("radio"===t.getAttribute("type")||null!==t.getAttribute("as-radio"))));for(const t of e){const e=t.getAttribute("value-source")||"checked";t[e]=!1}this.internals&&this.internals.setFormValue(this.checked?this.value:null)}}t.register("nn-input-radio",Ht);t.register("nn-input-range",class extends Bt{constructor(){super(),this.type="range"}});t.register("nn-input-search",class extends Bt{constructor(){super(),this.type="search"}});class qt extends(ut(pt(ct(ht(J))))){render(){return I`
